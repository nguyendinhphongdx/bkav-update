...
class UserController {
  ...
  async changeAvatar(req, res) {
    ...
    await userService
      .changeAvatar(idUser, req.file.filename, image, metadata)
      .then((user) => {
        responeInstance.success200(
          res,
          jsonInstance.toJsonWithData(`SUCCESS`, user)
    ...
    if (idUser && oldPassword && newPassword) {
      await userService
        ...
        .then((user) => {
          responeInstance.success200(
            res,
            jsonInstance.toJsonWithData(`change password successfully`, user)
          );
        })
        .catch((err) => {
          responeInstance.error400(res, jsonInstance.jsonNoData(err.message));
  ...
  async create(req, res) {
    ...
    await userService
      ...
      .then((user) => {
        responeInstance.success200(
          res,
          jsonInstance.toJsonWithData(`ADD SUCCCESS!`, user)
        );
      })
      .catch((err) => {
        responeInstance.error400(res, jsonInstance.jsonNoData(err.message));
